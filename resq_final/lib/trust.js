...
/**
 * Pseudonymous reporter id. We hash the IP with a per-process salt so repeat
 * submissions from one device can be spotted without ever storing the IP.
 */
const SALT = process.env.REPORTER_SALT || crypto.randomBytes(16).toString("hex");

export function reporterHash(req) {
  const ip =
